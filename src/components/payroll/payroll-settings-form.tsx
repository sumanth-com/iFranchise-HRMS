...
const processingDayLabels: Record<string, string> = {
  last_working_day: "Last working day",
  "25": "25th of month",
  "28": "28th of month",
  "1": "1st of next month",
...
function financialYearEndMonth(startMonth: number): number {
  return startMonth === 1 ? 12 : startMonth - 1;
}

function financialYearEndLabel(startMonth: number): string {
  const endMonth = financialYearEndMonth(startMonth);
  return new Date(2000, endMonth - 1, 1).toLocaleString("en-IN", { month: "long" });
}

type PayrollSettingsFormProps = {
  record: PayrollSettingsRecord;
  canEdit: boolean;
  /** Team Payroll tab: essential settings only. Company settings keeps the full form. */
  variant?: "full" | "team";
};
...
export function PayrollSettingsForm({
  ...
  function onSubmit(values: PayrollSettingsFormValues) {
    startTransition(async () => {
      ...
      const result = await savePayrollSettingsAction(payload);
      if (!result.success) {
  ...
  return (
    <form
      ...
      className={isTeam ? "flex flex-col" : "flex min-h-0 flex-1 flex-col gap-4"}
    >
      {isTeam ? (
        <div className="rounded-xl border bg-card shadow-sm">
          <div className="grid gap-3 border-b bg-muted/20 p-4 sm:grid-cols-2 lg:grid-cols-4">
            <TeamSettingsSnapshot
              icon={CalendarDays}
              label="Payroll cycle"
              value={cycleLabels[payrollCycle] ?? payrollCycle}
            />
            <TeamSettingsSnapshot
              icon={Wallet}
              label="Processing day"
              value={processingDayLabels[payrollProcessingDay] ?? payrollProcessingDay}
            />
            <TeamSettingsSnapshot
              icon={Coins}
              label="Salary credit"
          ...
          <div className="space-y-3 border-t px-5 py-4 md:px-6">
            {canEdit ? (
            ...
            <p className="text-center text-xs text-muted-foreground">
              Updated {format(new Date(record.audit.updatedAt), "MMM d, yyyy")}
              {record.audit.updatedByName ? ` · ${record.audit.updatedByName}` : ""}
            </p>
          </div>
        </div>
      ) : (
...
function TeamSettingsSnapshot({
  ...
  return (
    <div className="flex items-start gap-3 rounded-lg border bg-card px-3 py-2.5 shadow-sm">
