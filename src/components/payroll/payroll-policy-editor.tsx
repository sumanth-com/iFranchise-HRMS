...
type FormValues = Omit<PayrollPolicyDocument, "updatedAt">;
...
export function PayrollPolicyEditor({
  ...
  const onSubmit = handleSubmit((values) => {
    startTransition(async () => {
      const result = await savePayrollPolicyDocumentAction(values);
      if (!result.success) {
        toast.error(result.message);
        return;
      }
      const nextDocument: PayrollPolicyDocument = {
        ...values,
  ...
  return (
    <form onSubmit={onSubmit} className="flex w-full flex-col gap-6">
      ...
      <LeavePolicyPageHeader
        title="Edit Payroll Policy"
        description="Update the employee-facing payroll policy. Changes apply immediately after you click Update policy."
      />
