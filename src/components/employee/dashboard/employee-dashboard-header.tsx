...
function HeaderDateWidget({ now }: { now: Date | null }) {
  if (!now) {
    return <div className="h-[3.25rem] w-full max-w-[11.5rem] shrink-0 sm:w-[11.5rem]" aria-hidden />;
  }
  ...
  return (
    <div className="dashboard-surface flex w-full max-w-full shrink-0 items-center gap-3.5 rounded-xl border-0 bg-card px-4 py-2.5 sm:w-auto dark:shadow-none">
      ...
      <div className="min-w-0 leading-tight">
        <p
          className="truncate text-sm font-semibold tracking-tight text-foreground sm:whitespace-nowrap"
          suppressHydrationWarning
        >
          {format(now, "EEEE")}
        </p>
        <p
          className="mt-1 flex items-center gap-1.5 text-xs tabular-nums text-muted-foreground sm:whitespace-nowrap"
          suppressHydrationWarning
        >
...
export function EmployeeDashboardHeader({
  ...
  return (
    ...
    >
      ...
      <div className="relative flex min-w-0 flex-col gap-4 sm:flex-row sm:items-center sm:justify-between sm:gap-4">
        <div className="min-w-0 flex-1">
          <p className="mb-1.5 inline-flex items-center rounded-full bg-gradient-to-r from-blue-600/10 to-violet-600/10 px-2.5 py-0.5 text-[11px] font-medium tracking-wide text-violet-700 dark:text-violet-300">
            {resolvedSubtitle}
          </p>
          <h1 className="text-2xl font-semibold tracking-tight md:text-[1.75rem] md:leading-tight lg:text-[1.85rem] lg:leading-tight">
            {salutation}
            {showWave ? (
