...
import { managerDecideResignationAction } from "@/lib/exit/actions";
...
export function ManagerResignationsView({ pending }: Props) {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-2xl font-semibold tracking-tight">Team Resignations</h1>
        <p className="mt-1 text-sm text-muted-foreground">
          Review and approve resignation requests from your direct reports.
        </p>
      </div>
      ...
      <section className="rounded-xl border bg-card p-4 shadow-sm">
        <DataTable
          columns={columns}
          data={pending}
          emptyMessage="No pending resignations from your team."
        />
      </section>
      ...
      <Modal
        ...
        title={decisionType === "approve" ? "Approve resignation" : "Reject resignation"}
          ...
          <Button
            ...
            onClick={decisionForm.handleSubmit((values) => {
              startTransition(async () => {
                const result = await managerDecideResignationAction(values);
                if (!result.success) toast.error(result.message);
                else {
                  toast.success(
                    decisionType === "approve" ? "Resignation approved" : "Resignation rejected",
                  );
                  setSelected(null);
                  router.refresh();
                }
