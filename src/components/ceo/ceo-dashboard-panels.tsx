  ...
  return (
    <div className="flex min-h-0 flex-1 items-center gap-4">
      <div className="relative mx-auto shrink-0 sm:mx-0">
        <svg viewBox="0 0 100 100" className="size-28 -rotate-90 sm:size-32">
          <circle
            cx="50"
            cy="50"
            r={radius}
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <span className="text-2xl leading-none font-semibold tabular-nums">{total}</span>
      ...
      <ul className="min-h-0 min-w-0 flex-1 space-y-1.5 overflow-y-auto pr-0.5 text-xs sm:text-sm">
...
function Panel({
  ...
  return (
    <section className="flex h-full min-h-0 min-w-0 flex-col overflow-hidden rounded-xl border bg-card p-3 shadow-sm sm:p-4">
  ...
  return (
    ...
    >
      <div className="mb-2 grid shrink-0 grid-cols-3 gap-2">
        <div className="rounded-lg border bg-background/80 px-2 py-1.5">
        ...
        <div className="rounded-lg border bg-background/80 px-2 py-1.5">
          ...
          <p className="mt-0.5 text-base font-semibold tabular-nums">{attendance.absentToday}</p>
        </div>
        <div className="rounded-lg border bg-background/80 px-2 py-1.5">
          <p className="text-[9px] font-medium tracking-wide text-muted-foreground uppercase">
            Interviews
      ...
      <div className="flex min-h-0 flex-1 flex-col">
        <p className="mb-1.5 shrink-0 text-[9px] font-medium tracking-wide text-muted-foreground uppercase">
      ...
      <div className="mt-2 grid shrink-0 grid-cols-3 gap-2 border-t pt-2 text-center">
        <div>
          <p className="text-sm font-semibold tabular-nums">{organization.totalDepartments}</p>
          <p className="text-[9px] tracking-wide text-muted-foreground uppercase">Departments</p>
        </div>
        <div>
          <p className="text-sm font-semibold tabular-nums">{organization.totalManagers}</p>
          <p className="text-[9px] tracking-wide text-muted-foreground uppercase">Managers</p>
        </div>
        <div>
          <p className="text-sm font-semibold tabular-nums">
            {organization.reportingCoveragePercent}%
          </p>
          <p className="text-[9px] tracking-wide text-muted-foreground uppercase">Coverage</p>
        </div>
      </div>
    </Panel>
  );
...
function PriorityTile({
  ...
  return (
    <Link
      href={href}
      className="flex h-full min-h-0 flex-col justify-between rounded-lg border bg-background/80 px-3 py-2 transition-colors hover:border-primary/30 hover:bg-primary/[0.03]"
    >
      <div className="flex items-center justify-between gap-2">
        <span className="flex size-6 items-center justify-center rounded-md border bg-muted/40 text-muted-foreground">
          {icon}
        </span>
    ...
    <Panel title="Priorities" subtitle="Key decisions and company status" href={CEO_ROUTES.approvals}>
      <div className="grid h-full min-h-0 flex-1 grid-cols-2 gap-2">
        ...
        <PriorityTile
          ...
          detail={performance.pendingReviews > 0 ? "Still incomplete" : "No backlog"}
          href={CEO_ROUTES.performance}
          icon={<Target className="size-3.5" />}
          tone={
            performance.pendingReviews > 0 ? "text-amber-700 dark:text-amber-400" : undefined
          }
        />
        <PriorityTile
          label="Open Roles"
