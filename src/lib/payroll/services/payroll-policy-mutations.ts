...
export async function savePayrollPolicyDocument(
  supabase: AuthSupabaseClient,
  profile: UserProfile,
  document: Omit<PayrollPolicyDocument, "updatedAt">,
) {
  const stored: PayrollPolicyDocument = {
    ...document,
  ...
  await mergeOrganizationSettings(supabase, profile, (current) => ({
    ...current,
    payroll_policy_document: stored,
  }));
}
