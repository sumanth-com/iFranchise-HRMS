...
export async function saveAttendancePolicyDocument(
  supabase: AuthSupabaseClient,
  profile: UserProfile,
  document: Omit<AttendancePolicyDocument, "updatedAt">,
) {
  const stored: AttendancePolicyDocument = {
    ...document,
  ...
  await mergeOrganizationSettings(supabase, profile, (current) => ({
    ...current,
    attendance_policy_document: stored,
  }));
}
