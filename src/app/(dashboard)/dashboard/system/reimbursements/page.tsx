...
/**
 * Super Admin self-service reimbursements — own claims only.
 * Reuses the employee reimbursements UI/APIs; does not open admin/team queues.
 */
export default async function SuperAdminReimbursementsPage() {
  ...
  const [listResult, summary] = await Promise.all([
    safeServerCall(
      ...
      EMPTY_LIST,
      "[system-admin/reimbursements] list",
    ),
    safeServerCall(
      () => getReimbursementSummary(supabase, profile, { employeeId }),
      EMPTY_SUMMARY,
      "[system-admin/reimbursements] summary",
    ),
  ]);
