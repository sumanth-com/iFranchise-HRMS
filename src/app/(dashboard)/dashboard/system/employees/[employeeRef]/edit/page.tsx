...
const SYSTEM_EMPLOYEE_ROUTES = buildEmployeeModuleRoutes(SYSTEM_EMPLOYEE_LIST);
...
export default async function SuperAdminEmployeeEditPage({
  ...
  if (employeeRef !== canonicalRef || isEmployeeUuid(employeeRef)) {
    redirect(SYSTEM_EMPLOYEE_ROUTES.edit(resolved));
  }
        ...
        <div className="flex items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Update critical account or organization information for{" "}
              {employee.firstName} {employee.lastName}. Prefer HR for routine
              employment changes.
            </p>
          </div>
          <Link
            href={SYSTEM_EMPLOYEE_ROUTES.detail(employee)}
