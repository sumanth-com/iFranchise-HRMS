...
export default async function LeaveSettingsPage() {
  ...
  const [settings, hrApproverOptions] = await Promise.all([
    getCompanySettings(supabase, profile.employee.organizationId),
    listEligibleHrLeaveApproverOptions(profile.employee.organizationId),
  ]);
  ...
  return (
    <div className="flex min-h-full flex-1 flex-col gap-6">
      ...
      <LeavePoliciesForm
        ...
        canEdit={canEdit}
        hrApproverOptions={hrApproverOptions}
      />
    </div>
  );
}
