...
export default async function AccountantMyReimbursementsPage() {
  ...
  // Accountants may lack reimbursement.create but can still submit own claims via portal gate.
  const canCreate =
    hasPermission(profile.permissionCodes, "reimbursement.create") ||
    hasAnyPermission(profile.permissionCodes, [PORTAL_PERMISSIONS.accountant]);
  ...
  const [listResult, summary] = await Promise.all([
    safeServerCall(
      ...
      EMPTY_LIST,
      "[accountant/reimbursements] list",
    ),
    safeServerCall(
      () => getReimbursementSummary(supabase, profile, { employeeId }),
      EMPTY_SUMMARY,
      "[accountant/reimbursements] summary",
    ),
  ]);
