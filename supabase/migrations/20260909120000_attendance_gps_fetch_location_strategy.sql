-- Align GPS save gate with app: reject only absurd accuracy; always allow
-- fresh check-out writes (Update Check Out must replace previous out coords).
...
BEGIN
  ...
  -- Reject only clearly broken accuracy readings (e.g. ±200000 m).
  IF p_accuracy_m IS NOT NULL AND p_accuracy_m > 100000 THEN
    RETURN jsonb_build_object('ok', false, 'reason', 'absurd_accuracy');
  END IF;
  ...
  IF v_type = 'in' THEN
    ...
    -- Idempotent check-in retries: keep the better existing fix.
    IF FOUND
  ...
  ELSE
    -- Check-out / Update Check Out always stores the fresh capture.
    UPDATE hrms.attendance a
    SET
...
COMMENT ON FUNCTION hrms.self_service_attendance_save_location(uuid, text, double precision, double precision, double precision, uuid) IS
  'Persist GPS for own attendance row; reject only absurd accuracy; checkout always writes fresh coords.';
