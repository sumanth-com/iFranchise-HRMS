-- =============================================================================
-- Migration: hrms_ifranchise_employees_seed
-- Description: Seeds iFranchise workforce records with full employee profiles.
-- Run: SELECT hrms.seed_ifranchise_employees();
-- Idempotent: safe to re-run.
-- =============================================================================

CREATE OR REPLACE FUNCTION hrms.link_employee_auth_account(
  p_employee_id uuid,
  p_email text,
  p_first_name text,
  p_last_name text,
  p_employee_code text,
  p_role_code text DEFAULT 'employee'
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF v_user_id IS NULL THEN
    RETURN jsonb_build_object(
      ...
      'message', 'Auth user not found; employee record created without login access.'
    );
  END IF;
...
BEGIN
  ...
  LOOP
    ...
    VALUES (
      ...
      v_branch_id,
      v_rec.department_id,
      v_rec.designation_id,
      v_rec.employment_type_id,
      CASE
        WHEN v_rec.manager_code = 'IF2026001' THEN v_hr_lead_id
        ELSE v_manager_id
      END,
      v_rec.employee_code,
      v_rec.first_name,
    ...
    VALUES (
      ...
      v_rec.date_of_birth,
      v_rec.gender,
      'single'::hrms.marital_status,
      'Indian',
      'O+',
      lower(v_rec.email),
      v_rec.phone,
      'iFranchise team member onboarded via workforce seed.',
      'active'::hrms.record_status
    )
    ...
    SET deleted_at = public.utc_now()
    WHERE employee_id = v_effective_id
    ...
    INSERT INTO hrms.emergency_contacts (
      employee_id, name, relationship, phone, email, is_primary, status
    )
    VALUES (
      v_effective_id,
      v_rec.first_name || ' ' || v_rec.last_name || ' - Emergency Contact',
      'Family',
      v_rec.phone,
      lower(v_rec.email),
      true,
    ...
    INSERT INTO hrms.bank_accounts (
      employee_id, bank_name, account_holder_name, account_number, ifsc_code, branch_name, account_type, is_primary, status
    )
    VALUES (
      v_effective_id,
      'HDFC Bank',
      v_rec.first_name || ' ' || v_rec.last_name,
      'PENDING-' || v_rec.employee_code,
      'HDFC0001234',
      'Hyderabad - Madhapur',
      'salary',
      true,
    ...
    INSERT INTO hrms.salary_structures (
      employee_id, effective_from, currency_code,
      basic_salary, hra_amount, transport_allowance, other_allowances,
      tax_deduction, other_deductions, gross_salary, net_salary, components, status
    )
    VALUES (
      v_effective_id, v_rec.date_of_joining, 'INR',
      v_basic, v_hra, v_transport, 0, 0, 0, v_gross, v_gross,
      jsonb_build_object(
    ...
    SET deleted_at = public.utc_now()
    WHERE employee_id = v_effective_id
      AND document_type_id IN (v_doc_aadhaar, v_doc_pan)
      AND deleted_at IS NULL;
    ...
    VALUES
      (
        v_effective_id, v_doc_aadhaar, 'Aadhaar - ' || v_rec.aadhaar_number,
        'seed/' || v_org_id::text || '/' || v_effective_id::text || '/aadhaar-reference.txt',
        'aadhaar-' || v_rec.employee_code || '.txt', 'text/plain', 1, 'verified', 'active'
      ),
      (
        v_effective_id, v_doc_pan, 'PAN - ' || v_rec.pan_number,
        'seed/' || v_org_id::text || '/' || v_effective_id::text || '/pan-reference.txt',
        'pan-' || v_rec.employee_code || '.txt', 'text/plain', 1, 'verified', 'active'
      );

    DELETE FROM hrms.attendance
    WHERE employee_id = v_effective_id
      AND attendance_date >= v_rec.date_of_joining
    ...
    SELECT
      ...
      v_effective_id,
      day_series::date,
      day_series + time '09:30:00',
      day_series + time '18:30:00',
      'present'::hrms.attendance_status,
      8,
      'active'::hrms.record_status
    FROM generate_series(v_rec.date_of_joining, CURRENT_DATE - 1, interval '1 day') AS day_series
    WHERE extract(isodow from day_series) < 6
      AND v_rec.date_of_joining < CURRENT_DATE;

    IF NOT EXISTS (
      SELECT 1
      FROM hrms.audit_logs al
      WHERE al.table_name = 'employees'
        AND al.record_id = v_effective_id::text
        AND al.new_record ->> 'event' = 'employee_onboarded'
      ...
      VALUES (
        ...
        'employees',
        v_effective_id::text,
        'INSERT'::hrms.audit_operation,
        jsonb_build_object(
          'event', 'employee_onboarded',
          'employee_code', v_rec.employee_code,
          'full_name', v_rec.first_name || ' ' || v_rec.last_name,
          'date_of_joining', v_rec.date_of_joining
        ),
        v_rec.date_of_joining::timestamptz,
        'active'::hrms.record_status
      );
    END IF;

    v_auth_results := v_auth_results || jsonb_build_array(
      hrms.link_employee_auth_account(
        v_effective_id,
        v_rec.email,
        v_rec.first_name,
        v_rec.last_name,
  ...
  RETURN jsonb_build_object(
    'success', true,
    'employees_seeded', v_count,
    'auth_link_results', v_auth_results
  );
END;
$$;

COMMENT ON FUNCTION hrms.seed_ifranchise_employees() IS
  'Seeds the iFranchise workforce with full employee profiles, payroll, leave, and optional auth linking.';

COMMENT ON FUNCTION hrms.link_employee_auth_account(uuid, text, text, text, text, text) IS
  'Links an existing Supabase Auth user to an employee record and assigns a role.';

REVOKE ALL ON FUNCTION hrms.seed_ifranchise_employees() FROM PUBLIC;
REVOKE ALL ON FUNCTION hrms.link_employee_auth_account(uuid, text, text, text, text, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION hrms.seed_ifranchise_employees() TO service_role;
GRANT EXECUTE ON FUNCTION hrms.link_employee_auth_account(uuid, text, text, text, text, text) TO service_role;
