-- Clear stale reverse-geocoded address when GPS coords are rewritten.
-- Keeps punch/status/hours untouched.
...
BEGIN
  ...
  IF v_type = 'in' THEN
    ...
    SET
      ...
      -- Force fresh reverse-geocode on next location page view.
...
COMMENT ON FUNCTION hrms.self_service_attendance_save_location(uuid, text, double precision, double precision, double precision) IS
  'Best-effort GPS persist for self punch; writes only location columns on the caller own attendance row; clears address cache when coords change.';
