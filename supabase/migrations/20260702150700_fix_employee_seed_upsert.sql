-- Fix workforce seed to upsert existing employees by code/email (idempotent re-run).
...
BEGIN
  ...
  FOR v_rec IN
    ...
    FROM (
      ...
      designation_id, department_id, employment_type_id, date_of_joining,
      probation_end, date_of_birth, aadhaar_number, pan_number, earned_leave,
      is_intern, manager_code, gender
    )
  LOOP
    v_effective_id := v_rec.employee_id;

    SELECT e.id INTO v_effective_id
    FROM hrms.employees e
    WHERE e.organization_id = v_org_id
      AND e.employee_code = v_rec.employee_code
      AND e.deleted_at IS NULL
    LIMIT 1;
    ...
    SELECT e.id INTO v_hr_lead_id
    FROM hrms.employees e
    WHERE e.organization_id = v_org_id
      AND e.employee_code = 'IF2026001'
      AND e.deleted_at IS NULL
    LIMIT 1;
    ...
    v_status := CASE
      ...
      ELSE 'active'::hrms.employment_status
    END;

    IF v_rec.is_intern THEN
      v_basic := 12000; v_hra := 0; v_transport := 1500;
    ELSE
      v_basic := 30000; v_hra := 12000; v_transport := 2000;
    END IF;
    v_gross := v_basic + v_hra + v_transport;

    INSERT INTO hrms.employees (
      id, organization_id, branch_id, department_id, designation_id,
      employment_type_id, reporting_manager_id, employee_code, first_name,
      last_name, email, phone, employment_status, date_of_joining, status
    ) VALUES (
      v_effective_id, v_org_id, v_branch_id, v_rec.department_id, v_rec.designation_id,
      v_rec.employment_type_id,
      CASE WHEN v_rec.manager_code = 'IF2026001' THEN v_hr_lead_id ELSE v_manager_id END,
      v_rec.employee_code, v_rec.first_name, v_rec.last_name, lower(v_rec.email),
      v_rec.phone, v_status, v_rec.date_of_joining, 'active'
    )
    ON CONFLICT (id) DO UPDATE SET
      branch_id = EXCLUDED.branch_id, department_id = EXCLUDED.department_id,
      designation_id = EXCLUDED.designation_id, employment_type_id = EXCLUDED.employment_type_id,
      reporting_manager_id = EXCLUDED.reporting_manager_id, employee_code = EXCLUDED.employee_code,
      first_name = EXCLUDED.first_name, last_name = EXCLUDED.last_name, email = EXCLUDED.email,
      phone = EXCLUDED.phone, employment_status = EXCLUDED.employment_status,
      date_of_joining = EXCLUDED.date_of_joining, status = EXCLUDED.status,
      updated_at = public.utc_now(), deleted_at = NULL;

    INSERT INTO hrms.employee_profiles (
      employee_id, date_of_birth, gender, marital_status, nationality,
      blood_group, personal_email, personal_phone, bio, status
    ) VALUES (
      v_effective_id, v_rec.date_of_birth, v_rec.gender, 'single', 'Indian', 'O+',
      lower(v_rec.email), v_rec.phone,
      'iFranchise team member onboarded via workforce seed.', 'active'
    )
    ON CONFLICT (employee_id) DO UPDATE SET
      ...
      blood_group = EXCLUDED.blood_group, personal_email = EXCLUDED.personal_email,
      personal_phone = EXCLUDED.personal_phone, bio = EXCLUDED.bio,
      updated_at = public.utc_now(), deleted_at = NULL;

    UPDATE hrms.employee_addresses SET deleted_at = public.utc_now()
    WHERE employee_id = v_effective_id AND deleted_at IS NULL;
    INSERT INTO hrms.employee_addresses (
      employee_id, address_type, address_line1, address_line2, city, state, postal_code, country, is_primary, status
    ) VALUES
    ...
    UPDATE hrms.emergency_contacts SET deleted_at = public.utc_now()
    WHERE employee_id = v_effective_id AND deleted_at IS NULL;
    ...
    UPDATE hrms.bank_accounts SET deleted_at = public.utc_now()
    WHERE employee_id = v_effective_id AND deleted_at IS NULL;
    ...
    UPDATE hrms.salary_structures SET deleted_at = public.utc_now()
    WHERE employee_id = v_effective_id AND deleted_at IS NULL;
    INSERT INTO hrms.salary_structures (
      employee_id, effective_from, currency_code, basic_salary, hra_amount, transport_allowance,
      other_allowances, tax_deduction, other_deductions, gross_salary, net_salary, components, status
    ) VALUES (
      v_effective_id, v_rec.date_of_joining, 'INR', v_basic, v_hra, v_transport, 0, 0, 0, v_gross, v_gross,
      jsonb_build_object(
        'probation_end_date', v_rec.probation_end::text,
        'aadhaar_number', v_rec.aadhaar_number,
        'pan_number', v_rec.pan_number,
        'uan', 'PENDING', 'pf_number', 'PENDING', 'esi_number', 'PENDING'
      ), 'active'
    );

    INSERT INTO hrms.leave_balances (
      employee_id, leave_type_id, balance_year, allocated_days, used_days, pending_days, balance_days, status
    ) VALUES (
      v_effective_id, v_leave_el, 2026, GREATEST(v_rec.earned_leave, 0), 0, 0, GREATEST(v_rec.earned_leave, 0), 'active'
    )
    ON CONFLICT (employee_id, leave_type_id, balance_year) DO UPDATE SET
      allocated_days = EXCLUDED.allocated_days, used_days = EXCLUDED.used_days,
      pending_days = EXCLUDED.pending_days, balance_days = EXCLUDED.balance_days,
      updated_at = public.utc_now(), deleted_at = NULL;

    UPDATE hrms.employee_documents SET deleted_at = public.utc_now()
    WHERE employee_id = v_effective_id AND document_type_id IN (v_doc_aadhaar, v_doc_pan) AND deleted_at IS NULL;
    INSERT INTO hrms.employee_documents (
      employee_id, document_type_id, title, storage_path, file_name, mime_type, file_size_bytes, document_status, status
    ) VALUES
    ...
    DELETE FROM hrms.attendance
    WHERE employee_id = v_effective_id AND attendance_date >= v_rec.date_of_joining AND attendance_date < CURRENT_DATE;
    INSERT INTO hrms.attendance (
      organization_id, branch_id, employee_id, attendance_date, check_in_at, check_out_at, attendance_status, work_hours, status
    )
    SELECT v_org_id, v_branch_id, v_effective_id, day_series::date,
      day_series + time '09:30:00', day_series + time '18:30:00', 'present', 8, 'active'
    ...
    IF NOT EXISTS (
      SELECT 1 FROM hrms.audit_logs al
      WHERE al.table_name = 'employees' AND al.record_id = v_effective_id::text
        AND al.new_record ->> 'event' = 'employee_onboarded' AND al.deleted_at IS NULL
    ) THEN
      INSERT INTO hrms.audit_logs (organization_id, schema_name, table_name, record_id, operation, new_record, occurred_at, status)
      VALUES (v_org_id, 'hrms', 'employees', v_effective_id::text, 'INSERT',
        jsonb_build_object('event', 'employee_onboarded', 'employee_code', v_rec.employee_code,
          'full_name', v_rec.first_name || ' ' || v_rec.last_name, 'date_of_joining', v_rec.date_of_joining),
        v_rec.date_of_joining::timestamptz, 'active');
    END IF;

    v_auth_results := v_auth_results || jsonb_build_array(
      hrms.link_employee_auth_account(v_effective_id, v_rec.email, v_rec.first_name, v_rec.last_name, v_rec.employee_code,
        CASE WHEN v_rec.employee_code = 'IF2026001' THEN 'hr_admin' ELSE 'employee' END)
      || jsonb_build_object('employee_code', v_rec.employee_code)
    );
    v_count := v_count + 1;
  END LOOP;
