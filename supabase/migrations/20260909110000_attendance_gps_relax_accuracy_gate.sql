-- Relax GPS accuracy gate: reject only absurd values (e.g. ±200000 m),
-- not normal indoor / desktop browser accuracy.
...
BEGIN
  ...
  -- Reject only clearly broken accuracy readings, not real-world indoor GPS.
  IF p_accuracy_m IS NOT NULL AND p_accuracy_m > 50000 THEN
...
COMMENT ON FUNCTION hrms.self_service_attendance_save_location(uuid, text, double precision, double precision, double precision, uuid) IS
  'Persist GPS for the signed-in user own attendance row; reject only absurd accuracy; never replace a better fix with a poorer one.';
