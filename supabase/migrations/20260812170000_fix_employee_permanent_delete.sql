-- =============================================================================
-- Migration: fix_employee_permanent_delete
-- Description: Correct 1:1 action FK (meeting_id) and clear remaining portal
--              records so the same person can be added again as a new employee.
-- =============================================================================
...
BEGIN
  ...
  SET
    status = CASE
      WHEN employee_id = p_employee_id
        AND status NOT IN ('cancelled', 'archived', 'rejected', 'completed')
      THEN 'cancelled'::hrms.onboarding_status
      ELSE status
    END,
    cancelled_at = CASE
      WHEN employee_id = p_employee_id
      ...
      ELSE cancelled_at
    END,
    employee_id = CASE WHEN employee_id = p_employee_id THEN NULL ELSE employee_id END,
    reporting_manager_id = CASE
      WHEN reporting_manager_id = p_employee_id THEN NULL ELSE reporting_manager_id
    END,
    hr_reviewer_id = CASE
      WHEN hr_reviewer_id = p_employee_id THEN NULL ELSE hr_reviewer_id
    END,
    updated_at = public.utc_now()
  WHERE organization_id = p_organization_id
    AND (
      employee_id = p_employee_id
      OR reporting_manager_id = p_employee_id
      OR hr_reviewer_id = p_employee_id
      OR lower(trim(personal_email)) = lower(trim(v_employee.email::text))
    );

  DELETE FROM hrms.email_approval_tokens
  ...
  IF v_employee.user_id IS NOT NULL THEN
    DELETE FROM hrms.notification_user_preferences
    WHERE user_id = v_employee.user_id
      AND organization_id = p_organization_id;

    DELETE FROM hrms.user_preferences
    WHERE user_id = v_employee.user_id
      AND organization_id = p_organization_id;

    DELETE FROM hrms.user_login_sessions
    WHERE user_id = v_employee.user_id
      AND organization_id = p_organization_id;
  END IF;
