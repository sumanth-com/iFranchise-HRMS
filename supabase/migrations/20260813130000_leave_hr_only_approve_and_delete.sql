-- Leave: HR-only approve/reject, soft-delete permission, strip manager/executive approve rights.

INSERT INTO hrms.permissions (code, module, action, resource, description, status)
SELECT 'leave.delete', 'leave', 'delete', 'leave', 'Delete leave requests', 'active'::hrms.record_status
WHERE NOT EXISTS (
  SELECT 1 FROM hrms.permissions p WHERE p.code = 'leave.delete' AND p.deleted_at IS NULL
);
...
WHERE r.code IN ('hr_admin', 'super_admin')
  AND p.code = 'leave.delete'
...
-- Managers and executives no longer approve/reject leave — HR only.
...
WHERE rp.role_id = r.id
  ...
  AND rp.deleted_at IS NULL
  AND r.deleted_at IS NULL
  AND r.code IN ('manager', 'ceo', 'founder', 'co_founder')
...
COMMENT ON FUNCTION hrms.soft_delete_leave_request(uuid) IS
  'Soft-deletes a leave request and related approvals; restores leave balance when pending or approved.';

REVOKE ALL ON FUNCTION hrms.soft_delete_leave_request(uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION hrms.soft_delete_leave_request(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION hrms.soft_delete_leave_request(uuid) TO service_role;
